...
const handler = async (m, {command, conn}) => {
  ...
  if (command == 'شبح') {
    const anu = await wallpaper('Danny Phantom');
    const result = anu[Math.floor(Math.random() * anu.length)];
    const haha = result.image[0];
    await conn.reply(m.chat, global.wait, m);
    conn.sendMessage(m.chat, {image: {url: haha}, caption: `_${command}_`.trim()}, {quoted: m});
...
handler.command = ['الطبيعة', 'ببجي', 'جيمنج', 'شبح', 'wprandom', 'قهوة', 'نسكافية', 'كرتون', 'فضاء', 'تكنولوجيا', 'دورايمون', 'هكر', 'كوكب', 'randomprofile', 'wpaesthetic2', 'wpvehiculo', 'wallhp', 'wpmoto'];
